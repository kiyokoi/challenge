...
iter_05 = pd.read_csv('State_Drug_Utilization_Data_2005.csv', iterator=True, chunksize=5000)
df05 = pd.concat([chunk[(chunk['Suppression Used'] == False) & (chunk['State'] != 'XX')] for chunk in iter_05])
print df05.shape # (1291403, 20)

iter_07 = pd.read_csv('State_Drug_Utilization_Data_2007.csv', iterator=True, chunksize=5000)
df07 = pd.concat([chunk[(chunk['Suppression Used'] == False) & (chunk['State'] != 'XX')] for chunk in iter_07])
print df07.shape # (979286, 20)

iter_09 = pd.read_csv('State_Drug_Utilization_Data_2009.csv', iterator=True, chunksize=5000)
df09 = pd.concat([chunk[(chunk['Suppression Used'] == False) & (chunk['State'] != 'XX')] for chunk in iter_09])
print df09.shape # (1085318, 20)

iter_11 = pd.read_csv('State_Drug_Utilization_Data_2011.csv', iterator=True, chunksize=5000)
df11 = pd.concat([chunk[(chunk['Suppression Used'] == False) & (chunk['State'] != 'XX')] for chunk in iter_11])
print df11.shape # (1596407, 20)

iter_13 = pd.read_csv('State_Drug_Utilization_Data_2013.csv', iterator=True, chunksize=5000)
df13 = pd.concat([chunk[(chunk['Suppression Used'] == False) & (chunk['State'] != 'XX')] for chunk in iter_13])
print df13.shape # (1802170, 20)

iter_15 = pd.read_csv('State_Drug_Utilization_Data_2015.csv', iterator=True, chunksize=5000)
df15 = pd.concat([chunk[(chunk['Suppression Used'] == False) & (chunk['State'] != 'XX')] for chunk in iter_15])
print df15.shape # (1983852, 20)


# select top 10 reimbursed value
group = df15.groupby('Product Name').agg({'Total Amount Reimbursed':sum}).sort_values(by='Total Amount Reimbursed', ascending=False)
group = group.head(10)
print group.index
top10 = ['HARVONI 90', 'METHYLPHEN', 'ABILIFY', 'SOVALDI 40', 'LANTUS 3ML', 'ARIPIPRAZO', 'HUMIRA 40', 'TRUVADA 20', 'ADDERALL X', 'LANTUS 100']
...
df_list_top10 = [df05_top10, df07_top10, df09_top10, df11_top10, df13_top10, df15_top10]
for data in df_list_top10:
    df_top10 = df_top10.append(data)

print df_top10.shape  # (53409, 20)

df_total_reimbursed = df_top10.groupby(['Product Name', 'Year']).sum()[['Total Amount Reimbursed']].reset_index()
...
sns.barplot(x='Product Name', y='Total Amount Reimbursed', hue='Year', data=df_total_reimbursed)
...
df_total_reimbursed2 = df_exp.groupby('Year').sum()[['Total Amount Reimbursed']].reset_index()
...
df15_loc_total = df15_loc.groupby(['Latitude', 'Longitude']).sum()[['Total Amount Reimbursed']].reset_index()
df05_loc_total = df05_loc.groupby(['Latitude', 'Longitude']).sum()[['Total Amount Reimbursed']].reset_index()

plt.figure(figsize=(20,10))
img = plt.imread('Geo-map-USA-contour.png')
imgplot = plt.imshow(img, interpolation='bilinear', origin='upper', extent=[-126,-66,25,49.5])
plt.scatter(df15_loc_total['Longitude'], df15_loc_total['Latitude'], s=df15_loc_total['Total Amount Reimbursed']/20000000, edgecolors='b', facecolors='None')
plt.scatter(df05_loc_total['Longitude'], df05_loc_total['Latitude'], s=df05_loc_total['Total Amount Reimbursed']/20000000, edgecolors='r', facecolors='None')
plt.xlim([-126, -66])
plt.ylim([25, 49.5])
